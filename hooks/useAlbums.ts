...
export const albumQueryKeys = {
    ...
    posts: (albumId: number) => [...albumQueryKeys.all, 'posts', albumId] as const,
    public: (publicId: string) => [...albumQueryKeys.all, 'public', publicId] as const,
};
...
export const useAlbums = () => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!data.success) throw new Error(data.error ?? 'Failed to fetch albums');
...
export const useAlbumPosts = (albumId: number | null) => {
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (!data.success) throw new Error(data.error ?? 'Failed to fetch album posts');
...
export const useDeleteAlbum = () => {
    ...
    return useMutation({
        mutationFn: async (albumId: number) => {
            ...
            if (!data.success) throw new Error(data.error ?? 'Failed to delete album');
...
export const useUpdateAlbum = () => {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            if (!data.success) throw new Error(data.error ?? 'Failed to update album');
            return data as { success: true };
        },
...
export const useToggleAlbumVisibility = () => {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            return data as { success: true; is_public: boolean; public_id: string | null };
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: albumQueryKeys.list() });
        },
...
export const useAddPostToAlbum = () => {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            if (!data.success) throw new Error(data.error ?? 'Failed to add post to album');
            return data as { success: true; id: number };
        },
        onSuccess: (_data, { albumId }) => {
            queryClient.invalidateQueries({ queryKey: albumQueryKeys.posts(albumId) });
...
export const useRemovePostFromAlbum = () => {
    ...
    return useMutation({
        mutationFn: async ({
            ...
            if (!data.success) throw new Error(data.error ?? 'Failed to remove post from album');
            return data as { success: true };
        },
        onSuccess: (_data, { albumId }) => {
            queryClient.invalidateQueries({ queryKey: albumQueryKeys.posts(albumId) });
