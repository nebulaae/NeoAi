...
const ALBUM_GRADIENTS = [
    ...
    // plum → indigo
    'from-[#BF5AF2] via-[#5856D6] to-[#007AFF]',
    // coral → rose
    'from-[#FF6B6B] via-[#FF375F] to-[#BF5AF2]',
] as const;

// Decorative blob pattern SVG rendered inside the gradient
function GradientFallback({ albumId, name }: { albumId: number; name: string }) {
    const gradient = ALBUM_GRADIENTS[albumId % ALBUM_GRADIENTS.length];
    const initial = name.charAt(0).toUpperCase();

    return (
        <div className={cn('w-full h-full relative flex items-center justify-center bg-gradient-to-br', gradient)}>
            ...
            <div className="relative w-full aspect-[4/3] overflow-hidden">
                ...
                <div className="absolute top-2.5 right-2.5 flex items-center gap-1.5">
                    ...
                    {album.name !== 'Favorites' && (
                        ...
                        >
                            <Trash2 size={12} className="text-white/50 group-hover:text-white" />
                        </span>
                    )}
                </div>
            </div>
            ...
            <div className="px-4 py-3 flex items-center justify-between gap-2">
                <div className="flex flex-col min-w-0">
                    ...
                    <p className={cn(
                        ...
                        album.is_public ? 'text-[#007AFF]' : 'text-white/25'
                    )}>
    ...
    return (
        <div className="rounded-2xl overflow-hidden bg-zinc-900/40 border border-white/[0.06] group relative aspect-square">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-zinc-950/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-end p-3">
                <p className="text-[11px] font-black text-white/80 truncate">{post.model_name}</p>
            </div>
        </div>
    );
}
        ...
        <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-[18px] font-black truncate leading-tight">{album?.name ?? '...'}</p>
                ...
                {album?.is_public && album.public_id && (
                    <button
                        ...
                        className="w-10 h-10 rounded-2xl bg-[#007AFF]/10 border border-[#007AFF]/20 flex items-center justify-center active:scale-90 transition-all"
                    >
                        {copied ? (
                            <Check size={16} className="text-[#007AFF]" />
                        ) : (
                            <Copy size={16} className="text-[#007AFF]" />
                        )}
                    </button>
                )}
            </div>
            ...
            {isLoading ? (
                ...
                <div className="py-16 flex flex-col items-center justify-center gap-3 opacity-25">
                    <ImageIcon size={40} />
                    <p className="text-[13px] font-bold uppercase tracking-widest">{t('albumEmpty')}</p>
                </div>
            )}
        </div>
    );
...
export function AlbumsTab() {
    ...
    const handleTogglePublic = (e: React.MouseEvent, album: Album) => {
        ...
        toggleVisibility.mutate(
            ...
            {
                onSuccess: (data) => {
                    toast.success(data.is_public ? t('albumMadePublic') : t('albumMadePrivate'));
                },
        ...
        <div className="flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="flex items-center justify-between px-1">
                ...
                <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-[#007AFF]/10 border border-[#007AFF]/20 text-[#007AFF] text-[13px] font-black active:scale-95 transition-all hover:bg-[#007AFF]/20"
                >
                    <Plus size={14} />
                    {t('albumNew')}
                </button>
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-2 gap-3">
                    {albums.map((album) => (
                        <AlbumCard
                            key={album.id}
                            album={album}
                            onClick={() => { haptic.light(); setSelectedAlbumId(album.id); }}
                ...
                <div className="py-12 flex flex-col items-center justify-center gap-3 opacity-25">
                    <FolderOpen size={40} />
                    <p className="text-[13px] font-bold uppercase tracking-widest">{t('albumsEmpty')}</p>
                </div>
            )}
            ...
            >
                <DialogContent className="bg-zinc-950 border border-white/10 rounded-[28px] max-w-sm w-full p-6 gap-0 shadow-2xl">
                    ...
                    <input
                        ...
                        className="w-full px-5 py-4 rounded-2xl bg-zinc-900/60 border border-white/10 text-white text-[15px] font-bold placeholder:text-white/20 focus:outline-none focus:border-[#007AFF]/50 transition-colors mb-5"
                        autoFocus
                        maxLength={64}
                    />
                    ...
                    <DialogFooter className="flex flex-row gap-3 sm:justify-start">
                        ...
                        <button
                            ...
                            className="flex-1 py-3.5 rounded-2xl bg-[#007AFF] text-white text-[14px] font-black active:scale-95 transition-all disabled:opacity-40 shadow-[0_0_20px_rgba(0,122,255,0.3)]"
                        >
