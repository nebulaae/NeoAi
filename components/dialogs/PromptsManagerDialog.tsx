...
/** Миниатюра прикреплённого медиа */
function MediaThumb({
  item,
  onRemove,
}: {
  item: SavedPromptMedia;
  onRemove?: () => void;
}) {
  return (
    <div className="relative w-12 h-12 rounded-xl overflow-hidden border border-white/10 group shrink-0 bg-zinc-800">
      {item.type === 'image' ? (
        <img src={item.url} className="size-full object-cover" alt="" />
      ) : (
        <div className="size-full flex items-center justify-center text-white/40">
          {item.type === 'video' ? <Film size={18} /> : <Music size={18} />}
        </div>
      )}
      {onRemove && (
        <button
          ...
          className="absolute top-0.5 right-0.5 w-4 h-4 rounded-full bg-black/70 flex items-center justify-center text-white border border-white/20"
        >
          <X size={9} />
        </button>
      )}
      ...
      <DialogContent className="bg-zinc-950/95 border-white/10 text-white max-w-md p-0 rounded-[32px] backdrop-blur-2xl shadow-2xl overflow-hidden">
        ...
        <DialogHeader className="px-6 pt-6 pb-0">
          <div className="flex items-center gap-3 mb-1">
            <div className="w-9 h-9 rounded-2xl bg-[#007AFF]/20 border border-[#007AFF]/30 flex items-center justify-center shrink-0">
          ...
          {view === 'list' && (
            ...
            >
              ...
              <div className="px-6 pt-4 pb-3 flex gap-2">
                ...
                <button
                  onClick={openCreate}
                  className="w-10 h-10 rounded-2xl bg-[#007AFF] flex items-center justify-center shadow-[0_0_16px_rgba(0,122,255,0.4)] transition-all active:scale-90 shrink-0"
                >
                ...
                {filtered.length === 0 ? (
                  ...
                  filtered.map((p) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {p.media && p.media.length > 0 && (
                            <span className="flex items-center gap-0.5 text-[10px] font-bold text-[#007AFF] bg-[#007AFF]/10 px-1.5 py-0.5 rounded-md shrink-0">
                              <ImageIcon size={9} />
                              {p.media.length}
                            </span>
                          )}
                        ...
                        {/* Media thumbs preview */}
                        {p.media && p.media.length > 0 && (
                          <div className="flex gap-1.5 mt-2">
                            {p.media.slice(0, 4).map((m, i) => (
                              <div
                                key={i}
                                className="w-9 h-9 rounded-lg overflow-hidden border border-white/10 bg-zinc-800 shrink-0"
                              >
                                {m.type === 'image' ? (
                                  <img
                                    src={m.url}
                      ...
                      <ChevronRight
                        size={16}
                        className="text-[#007AFF] shrink-0 opacity-0 group-hover:opacity-100 transition-opacity mt-0.5"
                      />
                    </motion.div>
                  ))
                )}
            ...
            >
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-[15px] text-white placeholder:text-white/20 outline-none focus:border-[#007AFF]/40 transition-colors"
                  maxLength={60}
                />
              </div>
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <textarea
                  ...
                  className="bg-white/5 border border-white/10 rounded-2xl px-4 py-3 text-[15px] text-white placeholder:text-white/20 outline-none focus:border-[#007AFF]/40 transition-colors resize-none min-h-[120px] max-h-[220px]"
                  rows={5}
                />
              ...
              {/* Media field — закреплённые ссылки на медиа */}
              {formMedia.length > 0 && (
                <div className="flex flex-col gap-1.5">
                  <label className="text-[11px] font-black text-white/30 uppercase tracking-[0.12em] px-1">
                    Прикреплённые медиа ({formMedia.length})
                  </label>
