...
export function AddToAlbumDialog({ open, onClose, postId }: AddToAlbumDialogProps) {
            ...
            <DialogContent className="bg-zinc-950/95 border-white/10 text-white max-w-md p-0 rounded-[32px] backdrop-blur-2xl shadow-2xl overflow-hidden">
                ...
                <DialogHeader className="px-6 pt-6 pb-0 flex-row items-center gap-3">
                    <div className="w-9 h-9 rounded-2xl bg-[#007AFF]/20 border border-[#007AFF]/30 flex items-center justify-center shrink-0">
                        <FolderPlus size={17} className="text-[#007AFF]" />
                    </div>
                    ...
                    {mode === 'list' ? (
                        ...
                        >
                            ...
                            <button
                                ...
                                className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-[#007AFF]/8 hover:bg-[#007AFF]/14 border border-[#007AFF]/20 transition-all active:scale-[0.98] group"
                            >
                                <div className="w-10 h-10 rounded-xl bg-[#007AFF]/20 border border-[#007AFF]/25 flex items-center justify-center shrink-0">
                                    <Plus size={18} className="text-[#007AFF]" />
                                </div>
                                <p className="text-[14px] font-black text-[#007AFF]">Создать новый альбом</p>
                            </button>
                            ...
                            {isLoading ? (
                                ...
                                <div className="py-10 flex flex-col items-center gap-3 opacity-30">
                                    <FolderOpen size={32} />
                                    <p className="text-[12px] font-bold uppercase tracking-widest">Нет альбомов</p>
                                </div>
                            ) : (
                                albums.map((album) => {
                                    const isAdded = addedIds.has(album.id);
                                    const isLoading = loadingId === album.id;
                                    const gradient = ALBUM_GRADIENTS[album.id % ALBUM_GRADIENTS.length];
                                    ...
                                    return (
                                        ...
                                        >
                                            {/* Album thumb */}
                                            <div className={cn('w-10 h-10 rounded-xl overflow-hidden shrink-0 bg-gradient-to-br', gradient)}>
                                                {album.picture && (
                        ...
                        >
                            <input
                                ...
                                className="w-full px-4 py-3.5 rounded-2xl bg-white/5 border border-white/10 text-white text-[15px] font-bold placeholder:text-white/20 focus:outline-none focus:border-[#007AFF]/40 transition-colors"
                                maxLength={64}
                            />
                            <div className="flex gap-3">
                                ...
                                <button
                                    ...
                                    disabled={!newName.trim() || createAlbum.isPending || addPost.isPending}
                                    className="flex-1 py-3.5 rounded-2xl bg-[#007AFF] text-white text-[14px] font-black active:scale-95 transition-all disabled:opacity-40 shadow-[0_0_20px_rgba(0,122,255,0.3)] flex items-center justify-center gap-2"
                                >
