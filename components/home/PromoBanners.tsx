...
export function PromoBanners() {
  ...
  const banners: Banner[] = [
      ...
      ? [
          {
            ...
            cta: t('webCta'),
            icon: Globe,
            tint: 'from-[#007AFF]/35 via-[#007AFF]/10 to-transparent',
            glow: '0,122,255',
            onPress: () => {
              haptic.light();
    ...
    },
    {
      ...
      cta: t('feedCta'),
      icon: Flame,
      tint: 'from-orange-400/30 via-orange-400/8 to-transparent',
      glow: '251,146,60',
      onPress: () => go('/trends'),
    },
    {
  ...
  // Точки-индикаторы: активный слайд считаем по фактической позиции скролла,
  // а не по счётчику автолистания — иначе после ручного свайпа индикатор
  // показывал бы не тот баннер.
  const onScroll = () => {
          ...
          ({ id, kicker, title, cta, icon: Icon, tint, glow, onPress }) => (
            <button
              ...
              style={{
                ...
                boxShadow: `inset 0 0 42px -6px rgba(${glow},0.45), inset 0 1px 0 rgba(255,255,255,0.12)`,
              }}
              className={cn(
                'relative shrink-0 snap-start w-[86%] max-w-[420px] overflow-hidden rounded-3xl border bg-zinc-900/60 p-5 text-left transition-all active:scale-[0.98]'
              )}
            >
              <div
                className={cn(
              ...
              <div
                ...
                style={{
                  background: `radial-gradient(120% 90% at 88% 0%, rgba(${glow},0.42), transparent 62%)`,
                }}
              />
        ...
        {banners.map((b, i) => (
          <span
            ...
            className={cn(
              ...
              i === active ? 'w-5 bg-white/70' : 'w-1.5 bg-white/20'
            )}
          />
        ))}
      </div>
