...
export const Home = () => {
  ...
  return (
    <div className="pb-[calc(80px+max(16px,env(safe-area-inset-bottom)))] max-w-2xl mx-auto min-h-screen bg-black">
      {/* ── Nav ── */}
      <header
        className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 bg-black/60 backdrop-blur-2xl border-b border-white/5"
      >
        ...
        <div className='flex items-center gap-2'>
          ...
          <button
            ...
            className="flex items-center gap-1 px-4 py-2 rounded-full bg-[#AAFF00]/10 border border-[#AAFF00]/30 text-[#AAFF00] text-[13px] font-bold transition-all hover:scale-105 active:scale-95 shadow-[0_0_15px_rgba(170,255,0,0.3)]"
          >
      ...
      <section className="px-4 pt-6">
        <div className="flex items-center justify-between mb-6 px-2">
          <h2 className="text-[20px] font-bold text-white/90 tracking-tight">
            {t('trending')}
          </h2>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {postsLoading
            ...
            : posts.map((post: any) => {
              ...
              const isVideo = media?.type === 'video' || (media?.input && typeof media.input === 'string' && media.input.includes('.mp4'));
              ...
              return (
                <button
                  ...
                  className="group relative aspect-video rounded-3xl overflow-hidden bg-zinc-900 border border-white/10 transition-all duration-500 hover:scale-[1.02] active:scale-[0.98] hover:border-white/20 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
                >
                  {mediaUrl ? (
                    isVideo ? (
                      <video
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5 transform transition-transform duration-500">
                    ...
                    <h3 className="text-[16px] text-start font-bold text-white line-clamp-1 leading-tight group-hover:text-[#AAFF00] transition-colors">
                      {trendName}
                    </h3>
                  </div>
