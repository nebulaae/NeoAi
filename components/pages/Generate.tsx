...
import { ChevronLeft, Loader2, ImagePlus, X, Settings2, ChevronDown } from 'lucide-react';
...
export const Generate = () => {
  ...
  const models = (allModels || []).filter((m) => !m.categories?.includes('text'));
  ...
  const handleGenerate = () => {
    ...
    // Конвертируем в формат по документации: inputs.image[], inputs.video[], inputs.audio[]
  ...
  if (selected) {
    ...
    const canAttach = selected.input?.some((t) => ['image', 'video', 'audio'].includes(t));
    ...
    return (
      <div className="flex flex-col h-full pb-24">
        <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 bg-background/85 backdrop-blur-xl border-b border-border/50">
          <button
            onClick={() => { setSelectedTech(null); setPrompt(''); setMedia([]); setExtraParams({}); setShowParams(false); }}
          ...
          <div className="flex items-center gap-2">
            <Avatar className="size-6 rounded-lg">
              ...
              <AvatarFallback className="rounded-lg bg-secondary text-[10px] font-bold">{selected.model_name.slice(0, 2)}</AvatarFallback>
            </Avatar>
        ...
        <div className="flex flex-col gap-4 p-4 overflow-y-auto">
          ...
          {selected.versions && selected.versions.length > 1 && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Версия</p>
              <div className="flex flex-wrap gap-2">
                {selected.versions.map((v) => (
                  <button key={v.label} onClick={() => setSelectedVersion(v.label)}
                  ...
                  >
                    {v.label}<span className="ml-1 opacity-60">·{v.cost}💎</span>
                  </button>
                ))}
              </div>
            </div>
          ...
          <div>
            ...
            <Textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Опишите, что хотите создать..." className="resize-none min-h-28 bg-secondary/50 border-border/50 text-sm" />
          </div>

          {/* Aspect ratio */}
          {aspectParam && (
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Соотношение сторон</p>
              <div className="flex flex-wrap gap-2">
                {aspectParam.values?.map((val: string) => (
                  <button key={val} onClick={() => setExtraParams((p) => ({ ...p, aspect_ratio: val }))}
          ...
          {/* Advanced params */}
          {params && params.filter((p: any) => p.name !== 'aspect_ratio').length > 0 && (
            <div>
              <button onClick={() => setShowParams(!showParams)} className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors">
                <Settings2 className="size-3.5" />
                Дополнительные параметры
                <ChevronDown className={`size-3.5 transition-transform ${showParams ? 'rotate-180' : ''}`} />
              </button>
              {showParams && (
                <div className="mt-3 space-y-3">
                  {params.filter((p: any) => p.name !== 'aspect_ratio').map((p: any) => (
                    <div key={p.name}>
                      <label className="text-xs text-muted-foreground mb-1 block">{p.label || p.name}</label>
                      {p.type === 'select' && p.values ? (
                        <div className="flex flex-wrap gap-1.5">
                          {p.values.map((val: string) => (
                            <button key={val} onClick={() => setExtraParams((prev) => ({ ...prev, [p.name]: val }))}
                        ...
                        <input type="number" value={extraParams[p.name] ?? p.default ?? ''} min={p.min} max={p.max}
                          onChange={(e) => setExtraParams((prev) => ({ ...prev, [p.name]: Number(e.target.value) }))}
                          className="w-full px-3 py-1.5 rounded-lg bg-secondary border border-border text-sm"
                        />
                      ) : (
                        <input type="text" value={extraParams[p.name] ?? p.default ?? ''}
                          onChange={(e) => setExtraParams((prev) => ({ ...prev, [p.name]: e.target.value }))}
                          className="w-full px-3 py-1.5 rounded-lg bg-secondary border border-border text-sm"
                        />
                      )}
                    </div>
          ...
          {canAttach && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Медиафайл</p>
                <button onClick={() => fileInputRef.current?.click()} disabled={upload.isPending} className="flex items-center gap-1.5 text-xs text-primary font-medium disabled:opacity-50">
                  {upload.isPending ? <Loader2 className="size-3.5 animate-spin" /> : <ImagePlus className="size-3.5" />}
                  Прикрепить
                </button>
              </div>
              ...
              {media.length > 0 && (
                <div className="flex gap-2 flex-wrap">
                  {media.map((m, i) => (
                    <div key={i} className="relative size-20 rounded-xl overflow-hidden border border-border/50">
                      {m.type === 'image' ? (
                      ...
                      <button onClick={() => removeMedia(i)} className="absolute top-1 right-1 bg-black/60 rounded-full p-0.5 backdrop-blur-sm">
          ...
          <Button onClick={handleGenerate} disabled={(!prompt.trim() && media.length === 0) || generate.isPending || upload.isPending} className="w-full h-12 text-base font-semibold" size="lg">
            {generate.isPending ? <><Loader2 className="mr-2 size-5 animate-spin" />Запуск...</> :
              upload.isPending ? <><Loader2 className="mr-2 size-5 animate-spin" />Загрузка файла...</> : 'Создать'}
          </Button>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col h-full pb-24">
      <div className="sticky top-0 z-10 px-4 py-3 bg-background/85 backdrop-blur-xl border-b border-border/50">
        ...
        <p className="text-xs text-muted-foreground mt-0.5">Выберите нейросеть для генерации</p>
      </div>
      <div className="flex-1 overflow-y-auto">
        {isLoading ? <ModelsLoader /> : (
          <div className="flex flex-col">
            {(['image', 'video', 'audio'] as const).map((cat) => {
              ...
              const catLabel = cat === 'image' ? '🖼️ Изображения' : cat === 'video' ? '🎬 Видео' : '🎵 Аудио';
              return (
                <div key={cat}>
                  ...
                  {catModels.map((m) => {
                    const cost = m.versions?.find((v) => v.default)?.cost ?? m.versions?.[0]?.cost ?? 1;
                    return (
                      <button key={m.tech_name} onClick={() => setSelectedTech(m.tech_name)}
                      ...
                      >
                        <Avatar className="size-11 rounded-xl border border-border/50 shrink-0">
                          ...
                          <AvatarFallback className="rounded-xl bg-secondary text-xs font-bold">{m.model_name.slice(0, 2)}</AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold truncate">{m.model_name}</p>
                          {m.versions && m.versions.length > 1 && (
                            <p className="text-xs text-muted-foreground mt-0.5">{m.versions.length} версии</p>
                          )}
                        </div>
