...
const TrendCard = ({ post, onClick }: { post: Post; onClick: () => void }) => {
  ...
  const mediaUrl = media?.url || media?.input || (post.result as any)?.url;
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.96 }}
      onClick={onClick}
      className="group relative aspect-3/4 rounded-3xl overflow-hidden cursor-pointer border border-white/10 bg-zinc-900 transition-all duration-500 hover:border-white/20 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)]"
    >
      {mediaUrl ? (
        isVideo ? (
          <video
      ...
      <div className="absolute top-3 left-3 flex gap-2">
        {post.priority > 5 && (
          <div className="bg-[#007AFF] text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider shadow-lg">
            {t('new')}
          </div>
        )}
      </div>
