  ...
  >
    <div
      className={cn(
        ...
        spring,
        'active:scale-[0.975] hover:border-white/[.18] hover:bg-zinc-900/65'
      )}
    >
      <div
        className={cn(
...
const MaxIcon = () => (
  <svg width="20" height="20" viewBox="0 0 32 32" fill="none">
    <path
      d="M5 9h22M5 16h15M5 23h22"
      stroke="white"
      strokeOpacity="0.85"
      strokeWidth="3"
      strokeLinecap="round"
    />
    <circle cx="26" cy="16" r="4" fill="white" fillOpacity="0.45" />
  </svg>
);
...
export const Login = () => {
  ...
  const attemptTMALogin = useCallback(async () => {
    ...
    if (!initData) {
      console.warn(
        `[Login] initData not available (attempt ${retryCount.current}/${MAX_RETRIES})`
      );
  ...
  return (
    <PageWrapper>
      {/* Hero — title comes from outside this file, e.g. "NeoAI v1.1" or "Sibneuro" */}
      ...
      <div className="flex flex-col gap-3">
        {/* ── Browser: messenger deep-link cards ── */}
        {isBrowser && (telegramBotUsername || maxBotUsername) && (
          <>
        ...
        {/* ── Telegram widget (inside TG web app) ── */}
        {(isTg || isBrowser) && (
