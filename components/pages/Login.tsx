...
export const Login = () => {
  ...
  const botInfo = JSON.parse(typeof window !== 'undefined' ? localStorage.getItem('bot_info') || '{}' : '{}');
  ...
  const attemptTMALogin = useCallback(async () => {
    ...
    try {
      ...
      login(data.user);
      router.replace('/');
    } catch { setAutoLogging(false); attempted.current = false; }
      ...
      <div className="w-full max-w-[380px] flex flex-col flex-1">
        ...
        {view === 'main' ? (
          <div className="flex flex-col flex-1 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="flex flex-col items-center text-center mb-16 mt-8">
               <div className="w-24 h-24 rounded-[40px] bg-linear-to-br from-[#007AFF] to-[#0051FF] flex items-center justify-center mb-8 shadow-[0_20px_50px_rgba(0,122,255,0.3)] group overflow-hidden relative">
                  ...
                  <span className="text-[44px] relative z-10 drop-shadow-lg">✦</span>
               </div>
               <h1 className="text-[42px] font-black tracking-tighter leading-none mb-4 bg-linear-to-b from-white to-white/60 bg-clip-text text-transparent">NeoAI</h1>
               <p className="text-[17px] font-medium text-white/30 leading-relaxed max-w-[280px]">{t('tagline')}</p>
            </div>
            ...
            <div className="flex flex-col gap-4">
               ...
               <div className="p-6 rounded-[32px] bg-zinc-900/60 border border-white/5 flex flex-col gap-5 shadow-2xl">
                  <div className="flex items-center gap-3">
                     <div className="w-8 h-8 rounded-xl bg-[#229ED9]/10 flex items-center justify-center"><Image src="/telegram.png" width={18} height={18} alt="TG" /></div>
                     <span className="text-[15px] font-black uppercase tracking-widest text-white/40">{t('telegramSection')}</span>
                  </div>
                  {bot?.bot_username ? (
                    <div className="flex justify-center scale-110">
                       <LoginButton
                         botUsername={bot.bot_username}
                         onAuthCallback={(u) => api.post('/api/auth/telegram', {...u, bot_id: bot.bot_id}).then(res => {
               ...
               {/* Email Option */}
               <button onClick={() => { haptic.light(); setView('email-login'); }} className="p-5 rounded-[32px] bg-white/5 border border-white/10 flex items-center gap-4 transition-all hover:bg-white/10 active:scale-95 group">
                  ...
                  <div className="flex-1 text-left">
                     <p className="text-[16px] font-black tracking-tight">{t('emailLogin')}</p>
                     <p className="text-[12px] text-white/20 font-bold uppercase tracking-wider">{t('emailLoginSubtitle')}</p>
                  </div>
               ...
               <div className="mt-8 flex flex-col items-center gap-4">
                  <div className="flex items-center gap-6">
                     <Link href="/legal/offer" className="text-[13px] font-bold text-white/20 hover:text-[#007AFF] transition-colors">{tLegal('offer')}</Link>
                     <div className="w-1.5 h-1.5 rounded-full bg-white/5" />
                     <Link href="/legal/privacy" className="text-[13px] font-bold text-white/20 hover:text-[#007AFF] transition-colors">{tLegal('privacy')}</Link>
                  </div>
          ...
          <div className="flex flex-col animate-in fade-in slide-in-from-right-4 duration-500">
            <button onClick={() => setView('main')} className="flex items-center gap-2 text-white/40 font-bold mb-10 active:scale-90 transition-all">
            ...
            <div className="flex flex-col gap-4">
               ...
               >
                 {emailLoading ? <Loader2 className="animate-spin" /> : (view === 'email-login' ? t('signIn') : t('createAccount'))}
               </button>

               <button onClick={() => setView(view === 'email-login' ? 'email-register' : 'email-login')} className="mt-4 text-center text-white/30 font-bold text-[14px] hover:text-white transition-colors">
                  {view === 'email-login' ? t('noAccount') : t('alreadyHaveAccount')} <span className="text-[#007AFF]">{view === 'email-login' ? t('register') : t('signIn')}</span>
               </button>
            </div>
          </div>
        )}
