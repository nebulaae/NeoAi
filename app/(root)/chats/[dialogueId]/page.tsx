...
function extractResultMedia(result: Message['result']): Array<{ url: string; type: string }> {
...
export default function ChatPage({ params }: { params: { dialogueId: string } }) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setUploadedFiles((prev) => [...prev, { url: res.url, type: res.type, file: files[0] }]);
    } catch {
    ...
    generate.mutate(
      ...
      },
      {
        onSuccess: () => {
          setText('');
          setUploadedFiles([]);
          queryClient.invalidateQueries({ queryKey: queryKeys.chatHistory(params.dialogueId) });
        },
      }
    );
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-background">
      ...
      <div className="shrink-0 sticky top-0 z-10 flex items-center gap-3 px-4 py-3 bg-background/90 backdrop-blur-xl border-b border-border/50">
        <button onClick={() => router.back()} className="flex items-center justify-center size-8 rounded-full hover:bg-secondary/60 transition-colors active:scale-90">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 pb-6">
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-full text-center gap-3 py-16">
            ...
            <p className="text-sm text-muted-foreground">Начните диалог — напишите что-нибудь</p>
          </div>
        ) : (
          msgs.map((msg, idx) => {
                ...
                {(msg.inputs?.text || userMedia.length > 0) && (
                  <div className="flex justify-end">
                    <div className="bg-primary text-primary-foreground rounded-2xl rounded-tr-md px-3.5 py-2.5 max-w-[78%] text-sm">
                      {msg.inputs?.text && (
                        <p className="whitespace-pre-wrap leading-relaxed">{msg.inputs.text}</p>
                      )}
                      {userMedia.length > 0 && (
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          {userMedia.map((m, i) => (
                            <button key={i} onClick={() => setViewerSrc(m)}>
                              {m.type === 'image' ? (
                                <img src={m.url} alt="" className="max-h-36 rounded-lg object-cover" />
                              ) : m.type === 'video' ? (
                                <video src={m.url} className="max-h-36 rounded-lg" />
                              ) : (
                                <div className="flex items-center gap-1 text-xs opacity-80">🎵 Аудио</div>
                              )}
                            </button>
                          ))}
                        </div>
                ...
                <div className="flex justify-start">
                  <div className="bg-secondary/70 border border-border/40 text-foreground rounded-2xl rounded-tl-md px-3.5 py-2.5 max-w-[82%] text-sm">
                    {msg.status === 'processing' ? (
                      ...
                      <p className="text-red-400 text-sm">❌ {msg.error || 'Ошибка генерации'}</p>
                    ) : (
                      <>
                        {msg.result?.text && (
                          <p className="whitespace-pre-wrap leading-relaxed">{msg.result.text}</p>
                        )}
                          ...
                          <div className="mt-2 space-y-2">
                            {resultMedia.map((m, i) => (
                              <div key={i} className="relative group">
                                {m.type === 'image' ? (
                                  <>
                                    <img src={m.url} alt="" className="rounded-xl max-w-full cursor-pointer hover:opacity-95 transition-opacity" onClick={() => setViewerSrc(m)} />
                                  ...
                                  <video src={m.url} controls className="rounded-xl max-w-full" />
                                ) : m.type === 'audio' ? (
                                  <audio src={m.url} controls className="w-full mt-1" />
                                ) : null}
                              </div>
                            ))}
                          </div>
                        ...
                        {msg.cost && (
                          <p className="text-[10px] text-muted-foreground/50 mt-1.5 text-right">{msg.cost} 💎</p>
                        )}
                      </>
                    )}
                  </div>
      ...
      <div className="shrink-0 border-t border-border/50 bg-background/90 backdrop-blur-xl p-3">
        {uploadedFiles.length > 0 && (
          <div className="flex gap-2 mb-2 flex-wrap">
            {uploadedFiles.map((f, i) => (
              <div key={i} className="relative size-14 rounded-xl overflow-hidden border border-border/50">
                {f.type === 'image' ? (
                  <img src={URL.createObjectURL(f.file)} className="w-full h-full object-cover" alt="" />
                ) : (
                  <div className="w-full h-full bg-secondary flex items-center justify-center text-lg">
                    {f.type === 'video' ? '🎬' : '🎵'}
                  </div>
                )}
                <button onClick={() => removeFile(i)} className="absolute top-0.5 right-0.5 bg-black/60 rounded-full p-0.5">
        ...
        <div className="flex items-end gap-2 bg-secondary/50 rounded-2xl border border-border/40 px-2 py-2">
          <input ref={fileInputRef} type="file" accept="image/*,.heic,video/*,audio/*" onChange={handleFileUpload} className="hidden" />
          <button
          ...
          <button
            onClick={handleSend}
            disabled={(!text.trim() && uploadedFiles.length === 0) || generate.isPending || isProcessing}
      ...
      {/* Media viewer */}
      {viewerSrc && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4" onClick={() => setViewerSrc(null)}>
          <button className="absolute top-4 right-4 size-9 flex items-center justify-center rounded-full bg-white/10 text-white" onClick={() => setViewerSrc(null)}>
            <X className="size-5" />
          </button>
          {viewerSrc.type === 'image' ? (
            <img src={viewerSrc.url} alt="" className="max-w-full max-h-full rounded-xl object-contain" onClick={(e) => e.stopPropagation()} />
          ) : (
            <video src={viewerSrc.url} controls autoPlay className="max-w-full max-h-full rounded-xl" onClick={(e) => e.stopPropagation()} />
          )}
          <a href={viewerSrc.url} download target="_blank" rel="noreferrer"
