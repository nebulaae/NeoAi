    ...
    <div className="flex flex-col h-svh" style={{ background: 'var(--page-bg)' }}>
      ...
      <header className={cn(
        ...
        <button
          ...
          className={cn('flex items-center justify-center w-8.5 h-8.5 rounded-full shrink-0', g.thin, spring, 'active:scale-[0.88]')}>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
        {isHistoryLoading ? (
          ...
          <div className="flex flex-col items-center justify-center flex-1 gap-5 text-center py-10">
            <div className={cn('w-13 h-13 rounded-2xl flex items-center justify-center', g.regular)}>
                ...
                <div className="flex justify-start">
                  <div className="max-w-[82%]">
                    {msg.status === 'processing' || msg.status === 'pending' ? (
                      <div className={cn('flex items-center gap-2 px-3.5 py-2.5 rounded-[20px_20px_20px_4px]', g.regular)}>
                        {[0, 1, 2].map((i) => (
                      ...
                      <div className="flex flex-col gap-2">
                        {msg.result?.text && (
                          <div className={cn('px-3.5 py-2.5 rounded-[20px_20px_20px_4px]', g.regular, 'text-[15px] leading-normal whitespace-pre-wrap')}>
                            {msg.result.text}
                          </div>
                        )}
                              ...
                              if (m.type === 'audio') {
                                return (
                                  <div key={i} className="w-full max-w-[420px]">
                                    <div className={cn('flex flex-col gap-3 p-4', g.regular, 'rounded-3xl border border-white/20')}>
      ...
      {/* ── Input Bar ── */}
      <div className={cn('shrink-0', g.ultraThin,
          ...
          {canAttachMedia && (
            <>
              <button onClick={() => fileInputRef.current?.click()} disabled={upload.isPending}
                className={cn('shrink-0 w-9.5 h-9.5 flex items-center justify-center rounded-full', g.regular, spring, 'active:scale-[0.88]', upload.isPending && 'opacity-50')}>
