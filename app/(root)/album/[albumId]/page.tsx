...
function Skeleton({ className }: { className?: string }) {
    return (
        <div
            className={`animate-pulse bg-white/[0.06] rounded-2xl ${className}`}
        />
    );
}
                ...
                {posts.map((p, i) => {
                    ...
                    return (
                        <button
                            ...
                            className={`shrink-0 w-12 h-12 rounded-xl overflow-hidden transition-all duration-200 ${i === current
        ...
        <>
            <div className="min-h-screen bg-black text-white">
                ...
                <div className="px-3 pb-16">
                    {items.length === 0 ? (
                        <div className="flex flex-col items-center gap-3 py-24 text-white/20">
                            ...
                            <p className="text-[13px] font-medium uppercase tracking-widest">Пусто</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-3 gap-[3px]">
                            {items.map((post, index) => (
                                <PostCard
                                    ...
                                    post={post}
                                    index={index}
                                    onOpen={(p) => setLightboxIndex(items.findIndex((x) => x.id === p.id))}
                                />
                            ))}
                        </div>
                    )}
                ...
                <div className="pb-8 flex items-center justify-center gap-2 text-white/[0.12]">
                    <Sparkles size={12} />
                    <span className="text-[11px] font-semibold uppercase tracking-widest">AI Gallery</span>
                </div>
            </div>
